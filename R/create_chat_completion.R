...
create_chat_completion <-
  function(model, messages = NULL, functions = NULL,
    ...
    assertthat::assert_that(
      ...
      assertthat::noNA(temperature), value_between(
        temperature,
        0, 2
      )
    )
    assertthat::assert_that(
      assertthat::is.number(top_p), assertthat::noNA(top_p),
    ...
    assertthat::assert_that(
      assertthat::is.flag(stream), assertthat::noNA(stream),
      is_false(stream)
    )
    if (!is.null(stop)) {
      assertthat::assert_that(
        is.character(stop), assertthat::noNA(stop),
    ...
    assertthat::assert_that(
      ...
      assertthat::noNA(presence_penalty), value_between(
        presence_penalty,
        -2, 2
      )
    )
    assertthat::assert_that(
      ...
      assertthat::noNA(frequency_penalty), value_between(
        frequency_penalty,
        -2, 2
      )
    )
    ...
    body[["user"]] <- user
    response <- httr::POST(
      url = base_url, httr::add_headers(.headers = headers),
